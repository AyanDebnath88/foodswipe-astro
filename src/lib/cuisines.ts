 ...
 * one of the 9 seeded rows. It has no known dishes/dietary_tags, so it's
 * always shown (never dietary-filtered out) -- consistent with the AI
 * fallback only firing after the *known* deck is exhausted or stalled, at
 * which point showing an unfiltered wildcard card is better than nothing.
 */
